...
export default function Board() {
  ...
  const removingPieces = useGameStore((s) => s.removingPieces);

  const onlineBlocked = gameMode === 'online' && onlineRole && onlineRole !== currentPlayer;
  const inputLocked = aiThinking || onlineBlocked;

  const skin = getSkin(activeSkinState);

  // ── Sliding animation state ──
  // "ready" = mounted at from-position (no transition yet)
  // "sliding" = CSS transition active, moving to to-position
  const [slidePhase, setSlidePhase] = useState('idle'); // 'idle' | 'ready' | 'sliding'
  const rafRef = useRef(null);

  useEffect(() => {
    if (animatingMove) {
      setSlidePhase('ready');
      // Double rAF trick: first frame mounts element at 'from', second frame triggers transition
  ...
  // Each cell is 1/8 = 12.5% of the board.
  // translate() % is relative to the element itself (also 12.5% wide/tall),
  // so moving N cells = translate(N*100%, M*100%).
          ...
          <div className="relative grid grid-cols-8 grid-rows-8 aspect-square border-4 border-[var(--bg-primary)] flex-1 w-full h-full">
            ...
            {board.map((row, r) =>
              row.map((cell, c) => {
                ...
                const isSelected =
                  selectedPiece && selectedPiece[0] === r && selectedPiece[1] === c;
                ...
                const isAnimFrom = animatingMove &&
                  animatingMove.from[0] === r && animatingMove.from[1] === c;

                const removingPiece = removingPieces.find(p => p.r === r && p.c === c);

                const bgColor = isDark ? skin.darkCell : skin.lightCell;
            ...
            {onlineBlocked && (
              <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/25 pointer-events-none">
                <span className="arcade-text text-[9px] text-[var(--color-dim)] bg-[var(--bg-card)] pixel-border px-3 py-2">
                  ХОД СОПЕРНИКА
                </span>
              </div>
            )}
